...
class Nav
{
    ...
    public static function add($url, $label = null, $trail = null)
    {
        ...
        if (isset($r['fragment'])) {
            $parts[] = $r['fragment'];
            if (is_null($label)) {
                $label = Str::title($r['fragment']);
            }
        }
        if (empty($r['scheme'])) {
            //relative url found
            if (empty($url)) {
                $label = Str::title(static::$root);
                $url = static::$url;
            } else {
        ...
        if (is_null($label)) {
            $label = Str::title(strtok(end($parts), '.'));
        }
        $r['url'] = $url;
        $r['path'] = $trail;
        $r['parts'] = $parts;
